"""Initial rules for synthesis-record curation."""

from .times import normalize_time_hours

def clean_positive(input_path, output_path, *, plot_dir=None):
    """Apply the initial stage and write the resulting CSV."""
    ...
    def _split_pairs(values):
        # Empty selections still produce a valid, header-only table.
        return tuple(zip(*values)) if len(values) else ([], [])

    # Initial filtering and normalization of extraction records
    # Reports record counts and optional numeric distributions.
    ...
    def filled_series(s):
        return s.apply(is_filled).astype(bool)
    ...
    if not in_path.exists():
        raise FileNotFoundError("mof_extraction.csv not found")
    ...
    df["crystal_code"], _flags = _split_pairs(df["crystal_code"].apply(extract_digits_after_ccdc))
    ...
    for lk in ["linker_1", "linker_2", "linker_3"]:
        if lk in df.columns:
            before = df[lk].fillna("")
            df[lk], _flags = _split_pairs(df[lk].apply(remove_linker_trailing_parens))
            df[lk] = pd.Series(df[lk]).astype(str).str.strip()
            print(f"{lk} trailing parens removed: {count_changes(before, df[lk])}")

    # 4b.1) Merge linker synonyms after removing trailing parenthetical labels.
    ...
    # Fill unresolved reaction times while retaining reported numeric hours.
    for col in ["time_h", "time_text"]:
        if col not in df.columns:
            df[col] = np.nan
    before = df["time_h"].fillna("")
    df["time_h"] = [normalize_time_hours(value, text) for value, text in zip(df["time_h"], df["time_text"])]
    print(f"time_h filled from duration text: {count_changes(before, df['time_h'].fillna(''))}")
    ...
    for mc in metal_cols:
        before = df[mc].fillna("")
        df[mc], _flags = _split_pairs(df[mc].apply(normalize_x_to_middle_dot))
    ...
    def histogram_and_count(df, col, title):
        ...
        if count_nonnull > 0 and plot_dir is not None:
            import matplotlib.pyplot as plt
            plt.figure()
            vals.dropna().astype(float).plot(kind="hist", bins=20, title=title)
            plt.xlabel(col)
            plt.ylabel("Count")
            plt.savefig(Path(plot_dir) / f"{col}.png", bbox_inches="tight")
            plt.close()
...
def clean_negative(input_path, output_path, *, plot_dir=None):
    """Apply the initial stage and write the resulting CSV."""
    from pathlib import Path
    input_path, output_path = Path(input_path), Path(output_path)
    in_path = IN = IN_PATH = INOUT = input_path
    ...
    def _reset(df, why):
        ...
        # Verify that each filtered frame has a contiguous index.
    ...
    # drop neither
    df = df[~neither].copy()
    df = _reset(df, "drop rows with neither main_pdf nor si_pdf")  # Retain a contiguous row index.
    ...
    print(f"Dropping rows where '{col_atof}' is neither yes nor no: {int((~keep_atof).sum())}")
    df = df[keep_atof].copy()
    df = _reset(df, "article_trial_or_failure yes/no only")  # Retain a contiguous row index.
    print(f"Rows left: {len(df)}")
    ...
    print(f"Dropping rows where any of {required_cols} is empty: {int((~mask_required).sum())}")
    df = df[mask_required].copy()
    df = _reset(df, "required columns nonempty")  # Retain a contiguous row index.
    print(f"Rows left: {len(df)}")
    ...
    new_vals, _flags = _split_pairs(df["crystal_code"].apply(extract_digits_after_ccdc))
    ...
    for lk in ["linker_1", "linker_2", "linker_3"]:
        if lk in df.columns:
            before = df[lk].fillna("")
            vals, _flags = _split_pairs(df[lk].apply(remove_linker_trailing_parens))
    ...
    # Exclude slow evaporation and slow diffusion, while retaining slow cooling.
    ...
    print_header("temperature_c cleaning")
    print(f"Rows to drop due to temperature_c_text containing microwave or (slow) evaporation or slow diffusion: {int(drop_temp_text.sum())}")
    df = df[~drop_temp_text].copy()
    df = _reset(df, "temperature text screen")  # Retain a contiguous row index.

    still_empty_temp = ~filled_series(df["temperature_c"])
    print(f"Dropping rows with empty temperature_c after filling: {int(still_empty_temp.sum())}")
    df = df[~still_empty_temp].copy()
    df = _reset(df, "drop rows with empty temperature_c")  # Retain a contiguous row index.
    print(f"Rows left after temperature filters: {len(df)}")
    ...
    # ---------- pore_diameter_A cleanup ----------
    # Use a percentile cutoff to retain potentially valid large pores.
    if "pore_diameter_A" in df.columns:
        pore_vals = pd.to_numeric(df["pore_diameter_A"], errors="coerce")
