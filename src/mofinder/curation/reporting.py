...
def summarize(input_path, report_dir, *, top_n_metals=None, top_n_linkers=None):
    ...
    # Summaries, time/temperature distributions, and metal-linker coverage
    import pandas as pd
    import numpy as np
    import re
    from pathlib import Path
    ...
    # ---------------- config ----------------
    TOP_N_METALS   = top_n_metals   # e.g., 20
    TOP_N_LINKERS  = top_n_linkers   # e.g., 50
    PAIRS_CSV_BASENAME = str(Path(report_dir) / "metal_linker_pairs_report")  # output stem
    ...
    # Collect nonempty solvent names and abbreviations.
    solv_parts = []
    ...
    # concentration (mM, integer in the pipeline)
