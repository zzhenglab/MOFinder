"""Metal precursor normalization, unit conversion, and filtering."""

def clean(input_path, output_path, *, report_limit=50):
    """Normalize precursors, convert supported amounts, and write the resulting CSV."""
    from pathlib import Path
    input_path, output_path = Path(input_path), Path(output_path)
    in_path = IN = IN_PATH = INOUT = input_path
    out_path = OUT = OUT_PATH = output_path
    # Metal precursor normalization and pruning
    ...
    # Explicit formula and precursor mappings
    TEXT_SNAPS = {
        ...
        # Additional named precursor forms
    ...
    # ---------- unit conversion to mmol when possible ----------
    ...
    vc = m1.value_counts()
    print(f"\nTop {report_limit} metal_1:")
    print(vc.head(report_limit).to_string())

    print(f"\nLeast {report_limit} metal_1:")
    print(vc.sort_values(ascending=True).head(report_limit).to_string())
    return df
