...
Run the [data curation demo](../Demo/01_data_curation/README.md) with `--check` to compare generated records with its saved expected output. Each execution retains a run record and output snapshot.

The regression tests cover branch-specific filtering, formula masses and unit conversion, linker corrections, solvent density, derived features, and missing inputs. The [reaction-time tests](../tests/test_curation_times.py) check the duration conventions above and ensure that reported numeric hours remain unchanged. Pipeline tests verify that both curation branches apply those rules.
