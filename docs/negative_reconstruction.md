...
Negative reconstruction starts from the article and supporting-information manifest,
...
Edit `configs/negative_reconstruction.json`. Its paths are resolved relative to
...
input validation and document readability before negative planning. Each article and SI
...
python -m mofinder.extraction.negative mine --config configs/negative_reconstruction.json --dry-run
python -m mofinder.extraction.negative mine --config configs/negative_reconstruction.json
python -m mofinder.extraction.negative enumerate --config configs/negative_reconstruction.json --dry-run
...
The [data mining demo (API required)](../Demo/03_api_data_mining/README.md) uses these functions for a small
interactive run and preserves its positive records, plans, and enumerated outputs.

Negative planning selects only DOIs marked `yes` in the positive CSV, ignoring whitespace
...
be processed for planning but is excluded by the default YES-only enumeration gate. Validation
...
The source planning code sorts successful JSON files and removes duplicate JSON strings
...
Default negative planning resume behavior skips a DOI present in either its plan CSV or
...
unreadable existing plan CSV stops planning before any model requests; it is not
