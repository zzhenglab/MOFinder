...
Complete saved model-run artifacts are not yet included; historical whole-corpus Y/N labels are available in the [classification audit](data/metadata/literature_retrieval/classification_audit.csv). Statistical methods and prompt-development history are described in [the triage guide](docs/triage.md).
...
Literature retrieval includes article and SI input tables with rough topic classifications and a compact [DOI/classification table](data/metadata/literature_retrieval/doi_classification.csv) covering all 13,770 unique papers. These topic classifications are deterministic title/abstract/document-type analysis, **not generated by an LLM**, and have not been validated against expert topic labels. The revised Chemical synthesis category includes original papers reporting experimental MOF/framework preparation, even when structural analysis or application performance is the main topic. Earlier primary-topic categories, framework-synthesis evidence, saved model triage Y/N decisions, and uncertainty flags are retained in the [classification audit](data/metadata/literature_retrieval/classification_audit.csv). This changes the descriptive taxonomy; it does not establish improved screening accuracy.
