...
Literature retrieval includes two 7,437-row input tables with rough topic classifications and a compact [DOI/classification table](data/metadata/literature_retrieval/doi_classification.csv) covering all 13,770 unique papers. These topic classifications are deterministic title/abstract/document-type analysis, **not generated by an LLM**, and have not been validated against expert topic labels. Saved model triage Y/N decisions and uncertainty flags are listed separately in the [classification audit](data/metadata/literature_retrieval/classification_audit.csv). Published inventories omit download-status columns; the desktop applications maintain these states in local working copies. Research article and SI downloads remain local. The [demonstration PDFs](Demo/03_api_demo/inputs/mining/README.md) contain synthetic sample text and no measured experimental data. See [the literature retrieval input inventory](data/metadata/literature_retrieval/README.md) for methods, input hashes, and publisher-profile assignments.
