"""Small API examples for abstract triage and document mining."""
...
def validate_positive(config_dir=CONFIG_DIR):
    ...
    config = load_config(Path(config_dir) / "positive_mining.json")
    ...
    if report["missing_files"] or report["dois_without_text"]:
        raise ValueError(f"The mining documents need attention: {report}")
...
def validate_negative(config_dir=CONFIG_DIR):
    ...
    config = load_config(Path(config_dir) / "negative_mining.json")
...
def validate_all(config_dir=CONFIG_DIR):
    """Validate local inputs and create the selected mining manifest."""
    return {
        "triage": validate_triage(),
        "positive_mining": validate_positive(config_dir),
        "negative_mining": validate_negative(config_dir),
    }
...
def run_positive(config_dir=CONFIG_DIR):
    ...
    require_api_key()
    return run_from_config(Path(config_dir) / "positive_mining.json")
...
def run_negative(*, live=False, config_dir=CONFIG_DIR):
    ...
    if problems:
        raise ValueError(
            "Negative mining requires the positive extraction CSV and its synthesis "
...
def main(argv=None):
    ...
    parser.add_argument("--live", action="store_true",
                        help="Send model requests; negative mining also enumerates saved plans.")
    parser.add_argument("--config-dir", type=Path, default=CONFIG_DIR,
                        help="Directory containing the three mining JSON configurations; triage uses configs/triage.json.")
