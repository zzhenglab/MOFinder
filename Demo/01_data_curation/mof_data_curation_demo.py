"""Curate the bundled positive extraction records with the main curation package."""
...
def _run(config_file=DEMO_DIR / "config.json", *, output_dir=None):
    ...
    stages = []
    with TemporaryDirectory(prefix="mofinder_curation_") as directory:
    ...
    cleaned[preview_columns + ["M_L_ratio", "metel_concnertation"]].to_csv(output_dir / "processed_preview.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"Data curation: {len(raw)} input records; {len(cleaned)} processed records.")
    print(f"Output: {display_path(output_dir)}")
    return summary


def verify_outputs(output_dir=DEMO_DIR / "outputs"):
    """Compare actual and expected row counts and every processed value."""
...
def run(config_file=DEMO_DIR / "config.json", *, output_dir=None, check=False, history_dir=None):
    ...
    with DemoRun(DEMO_DIR, out, inputs, history_dir=history_dir) as record:
        ...
        if check:
            ...
            print("Expected data curation output:", "PASS" if record.verification["passed"] else "FAIL")
            if not record.verification["passed"]:
                raise AssertionError(f"Data curation output differs from expected; see {record.reference(record.record_path)}")
