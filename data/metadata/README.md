...
The [literature retrieval tables](literature_retrieval/README.md) include a compact
[DOI/classification export](literature_retrieval/doi_classification.csv) for all
13,770 unique papers. Topic classification is rough deterministic analysis of
titles, abstracts, and document types; **it is not generated by an LLM**. The
separate [classification audit](literature_retrieval/classification_audit.csv)
retains uncertain assignments, earlier primary-topic categories, framework
synthesis evidence, and existing model triage Y/N decisions. The revised
Chemical synthesis category includes original papers reporting experimental
MOF/framework preparation even when structure or application is the main topic.
This taxonomy change preserves the saved screening decisions; it does not
demonstrate improved screening accuracy.
