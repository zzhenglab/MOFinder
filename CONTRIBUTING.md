...
Use the [canonical workflow terminology](docs/workflow.md#workflow-terminology) in documentation, demo titles, and command descriptions. In particular, use **data curation** for processing synthesis records and **dataset preparation** for generating training and holdout JSONL. Distinguish **positive extraction** from **negative reconstruction**. Name processed artifacts consistently and identify JSONL separately from JSON. Preserve original source filenames and recorded run metadata when documenting provenance.
